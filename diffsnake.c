...
Point snake[40 * 20 + 1]; // Adjust based on initial width * height

int snake_length, food_x, food_y, direction,
    game_over = 0, acceleration = 0, width = 40, height = 20, delay = 100;
...
void update_snake()
{
    ...
    if (acceleration) delay = 180 - snake_length;                                                                   // Adjust delay dynamically, also this might be implemented wrong
}
...
void apply_difficulty(char difficulty)
{
    switch (difficulty)
    {
        case 'e': delay = 110; width += 5; height += 5; break;         // Easy
        case 'n': break;                                               // Default settings
        case 'h': delay = 90; width -= 5; height -= 5; break;          // Hard
        case 'a': acceleration = 1; delay = 180 - snake_length; break; // Acceleration
        case 'c': clear(); echo();
                  mvprintw(5, 10, "Enter width: "); scanw("%d", &width);
                  mvprintw(6, 10, "Enter height: "); scanw("%d", &height);
                  mvprintw(7, 10, "Enter delay (ms): "); scanw("%d", &delay);
                  mvprintw(8, 10, "Acceleration? (y/n): "); char acc; scanw(" %c", &acc);
                  acceleration = (acc == 'y' || acc == 'Y') ? 1 : 0; noecho(); break;
...
int main()
{
    ...
    while (direction == 0) // Wait for arrow key input
    {
